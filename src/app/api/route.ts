...
export async function POST(req: Request) {
  try {
    ...
    if (!openai) {
      ...
      if (mode === "mindCaptureAI") return NextResponse.json({ candidates: [], state: { labels: ["ローカル仕分け"], summary: "OPENAI_API_KEY未設定のため、アプリ側の軽量仕分けを使います。" } }, { status: 200 });
    ...
    if (mode === "mindCaptureAI") {
      const rawText = String(body.text || "").slice(0, 5000);
      if (!rawText.trim()) return NextResponse.json({ candidates: [], state: { labels: ["入力待ち"], summary: "入力がまだありません。" } });
      try {
        const completion = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: `あなたはLife Command OSのMind Capture分類AIです。ユーザーの自由入力を、保存前確認に使える候補へ分けます。JSONだけ返してください。
形式:
{
  "candidates": [
...
- 日本語で返す`,
        ...
        console.error(error);
        return NextResponse.json({ candidates: [], state: { labels: ["ローカル仕分け"], summary: "AI仕分けが少し詰まったため、アプリ側の軽量仕分けを使います。" } }, { status: 200 });
      }
    }
    ...
    const system =
      ...
      : mode === "budgetAI" ? "あなたはLife Command OSのMoney Hub AIです。収入・支出・チャージ・資金移動・財布/電子マネー残高・固定費・サブスク・カテゴリ予算・今日使える目安・月末予測を横断し、①今月の状態 ②今日あと使える目安 ③使いすぎそうなカテゴリ ④安心できる次の一手を、やさしく具体的に220〜360字で返してください。命令口調や責める表現は禁止です。診断ではなく生活整理の補助として話してください。"
      ...
      : mode === "searchAI" ? "あなたはLife Command OSのAI検索エンジンです。渡された検索語、検索結果候補、件数を見て、関連しそうなページと見る順番を短く具体的に返してください。結果候補に含まれる記録は引用してよいですが、存在しないデータは断定しないでください。返答は日本語で、やさしく、120〜220字。"
      ...
      : mode === "brainDumpAI" ? "Mind Captureの文章を、TODO・メモ・後で読む・予定候補に分けて、しゅうやくんのワーキングメモリ負担が軽くなるように整理してください。JSONでなく普通の日本語で短く。"
