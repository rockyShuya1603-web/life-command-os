...
function localAssist(body: string, mode: AssistMode, attachments: Array<{ name?: string; type?: string; kind?: string; text?: string }>) {
  ...
  if (mode === "TODO化") {
    return { ok: true, title, body, tags, category, todos: todos.length ? todos : [`${title}を確認する`], note: "TODO候補を作りました。" };
  }

  if (mode === "分類") {
    return { ok: true, title, body, tags, category, todos, note: `分類候補は「${category}」です。` };
  }
  ...
  if (mode === "補助") {
    return {
      ok: true,
      title,
      body: `${body}${attachmentLine}\n\n補助メモ:\n- 「${category}」系のメモとして扱えます\n- 迷ったら次の行動を1つだけTODOに変換すると使いやすいです`,
      tags,
      category,
      todos,
      note: "補助文を追加しました。",
...
export async function POST(req: NextRequest) {
  ...
  if (!body.trim() && attachments.length === 0) {
    return NextResponse.json({ ok: false, note: "本文か添付が必要です。" }, { status: 400 });
  }
  ...
  try {
    const prompt = [
      ...
      "注意: 画像の中身は直接読めない場合があるので、添付ファイル名やユーザー本文から推測しすぎないでください。",
    ].join("\n");
