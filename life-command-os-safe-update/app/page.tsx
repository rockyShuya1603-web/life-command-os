...
type PageKey =
  | "home" | "memos" | "tweets" | "todos" | "calendar" | "diary" | "coffee" | "budget" | "belongings" | "routines" | "trash" | "map"
...
function localGuide(data: Snapshot | null) {
  const today = todayKey();
  const todos = data?.todos.filter((t) => t.due_date === today && !t.done).length || 0;
  const caffeine = data?.coffee.filter((c) => c.drink_date === today).reduce((sum, c) => sum + Number(c.caffeine_mg || 0), 0) || 0;
  const routines = data?.routineChecks.filter((c) => c.check_date === today).length || 0;
  const recentDiary = data?.diaries?.[0]?.title || data?.diaries?.[0]?.content?.slice(0, 28) || "まだDiaryはこれから";
  const moneyToday = (data?.budget || []).filter((b) => b.spend_date === today && b.type === "expense").reduce((sum, b) => sum + Number(b.amount || 0), 0);
  const nextAction = todos ? "TODOを1つだけ5分版にする" : routines ? "達成ログを眺めて休む" : "脳ダンプに1行だけ置く";
  return `しゅうやくん、今日の流れを軽く読んだよ。\n\n未完了TODOは${todos}件、カフェインは約${caffeine}mg、習慣チェックは${routines}件。今日の支出は約${yen(moneyToday)}。\n\n最近のDiary候補は「${stripHtml(recentDiary)}」。今のおすすめは「${nextAction}」。全部を一気に片付けなくて大丈夫。まず1個だけ、手前に置こう。`;
}
function GuideAiCard({ themeKey, message, onRefresh, loading }: { themeKey: ThemeKey; message: string; onRefresh?: () => void; loading?: boolean }) {
  const theme = themes[themeKey];
  const lines = String(message || "").split(/\n+/).filter(Boolean);
  const main = lines[0] || "今日もおつかれさま。";
  const sub = lines.slice(1).join("\n\n") || "生活ログを見ながら、次の一歩を小さく整えるね。";
  return <div className="overflow-hidden rounded-[1.75rem] border border-white/10 bg-white/[0.08] shadow-2xl">
    <div className="relative h-72 bg-black/30 sm:h-80">
      <img src="/life-ai-guide.png" alt="案内係AI" className="h-full w-full object-cover" loading="lazy" />
      <div className="absolute inset-0 bg-gradient-to-t from-black via-black/15 to-transparent" />
      <div className="absolute bottom-4 left-4 right-4">
        <p className={`inline-flex rounded-full bg-gradient-to-r ${theme.accent} px-3 py-1 text-xs font-black text-black`}>癒し系案内AI v2</p>
        <h3 className="mt-2 text-2xl font-black">{main}</h3>
        <p className="mt-1 text-xs font-bold text-white/60">生活ログ・TODO・家計簿・Diaryをまとめて読む案内役</p>
      </div>
    </div>
    <div className="p-4 sm:p-5">
      <div className="grid gap-3 sm:grid-cols-[1fr_.72fr]">
        <div className="relative rounded-3xl border border-white/10 bg-black/30 p-4">
          ...
          <p className="whitespace-pre-wrap text-sm leading-7 text-white/82">{sub}</p>
        </div>
        <div className="rounded-3xl border border-white/10 bg-white/10 p-4">
          <p className="text-xs font-black text-cyan-100/80">今日の見方</p>
          <p className="mt-2 text-sm leading-6 text-white/70">① 頭の中を減らす<br />② お金のズレを小さくする<br />③ Diaryに物語として残す</p>
        </div>
      </div>
      {onRefresh && <button onClick={onRefresh} disabled={loading} className="mt-3 w-full rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-sm font-black transition active:scale-[0.99] disabled:opacity-50">{loading ? "読んでる..." : "今日の内容から話しかけてもらう"}</button>}
    </div>
  </div>;
}
...
function DiaryPanel({ snapshot, refreshSnapshot, setPage }: PanelProps) {
  ...
  const [blogTone, setBlogTone] = useState("日記");
  ...
  function insertDiarySnippet(snippet: string) {
  ...
  async function add() {
    ...
    const html = image ? `${content}<p><img src="${image}" /></p>` : content;
    ...
    setContent("");
    setImage("");
    setGuideDraft("Diaryを保存したよ。今日の一部がちゃんと物語として残ったね。");
    await refreshSnapshot();
  }
  ...
  return (
    <div className="space-y-4">
      <GlassCard>
        ...
        <p className="mt-2 text-sm text-white/60">ブログみたいに見出し・引用・区切り・写真を使って自由に残せるDiary。長押しすると読書ページでゆっくり読めるよ。</p>
      </GlassCard>
      <div className="grid gap-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <select value={blogTone} onChange={(e) => setBlogTone(e.target.value)} className="rounded-2xl border border-white/20 bg-slate-950/90 p-4 text-white">
            <option>日記</option><option>ブログ</option><option>エッセイ</option><option>記録</option><option>物語</option>
          </select>
        ...
        <div className="rounded-3xl border border-white/10 bg-black/20 p-3">
          <div className="mb-3 flex flex-wrap gap-2">
            <button type="button" onClick={() => insertDiarySnippet("<h1>今日の見出し</h1>")} className="rounded-2xl bg-white/10 px-3 py-2 text-xs font-black">大見出し</button>
            <button type="button" onClick={() => insertDiarySnippet("<h2>小さな章</h2>")} className="rounded-2xl bg-white/10 px-3 py-2 text-xs font-black">章</button>
            <button type="button" onClick={() => insertDiarySnippet("<blockquote>印象に残った言葉</blockquote>")} className="rounded-2xl bg-white/10 px-3 py-2 text-xs font-black">引用</button>
            <button type="button" onClick={() => insertDiarySnippet("<hr />")} className="rounded-2xl bg-white/10 px-3 py-2 text-xs font-black">区切り</button>
            <button type="button" onClick={() => insertDiarySnippet("<ul><li>できたこと</li><li>次に試すこと</li></ul>")} className="rounded-2xl bg-white/10 px-3 py-2 text-xs font-black">リスト</button>
            <button type="button" onClick={() => setPreviewOpen((v) => !v)} className="rounded-2xl bg-cyan-300 px-3 py-2 text-xs font-black text-black">{previewOpen ? "編集に戻る" : "プレビュー"}</button>
          </div>
          <p className="mb-2 text-xs text-white/45">形式: {blogTone} / 文字数 {wordCount} / 読了目安 {readMinutes}分</p>
          {!previewOpen ? <TextArea className="h-56" placeholder="本文。見出し・引用・リスト・写真でブログ風に残せるよ。" value={content} onChange={(e) => setContent(e.target.value)} /> : <article className="diary-content min-h-56 rounded-3xl bg-slate-950/70 p-5 text-white/88" dangerouslySetInnerHTML={{ __html: image ? `${content}<p><img src=\"${image}\" /></p>` : content || "<p>ここにプレビューが出るよ。</p>" }} />}
        </div>
      ...
      {readMenuTarget && (
        <Modal title="Diaryメニュー" onClose={() => setReadMenuTarget(null)}>
          <div className="space-y-3">
            <p className="text-sm text-white/60">{readMenuTarget.entry_date} / {readMenuTarget.title || "Diary"}</p>
            <button
              onClick={() => { setReaderTarget(readMenuTarget); setReadMenuTarget(null); }}
              className="w-full rounded-2xl bg-white px-4 py-3 font-black text-black"
            >
              日記を読む
            </button>
            <button onClick={() => setReadMenuTarget(null)} className="w-full rounded-2xl bg-white/10 px-4 py-3 font-bold">閉じる</button>
          </div>
        </Modal>
      )}
    </div>
...
function CoffeePanel({ snapshot, refreshSnapshot }: PanelProps) {
  ...
  const presets = [
    ...
    { name: "カフェラテ", mg: 80 },
    { name: "エスプレッソ", mg: 65 },
    { name: "エナジードリンク", mg: 140 },
    { name: "デカフェ", mg: 10 },
  ];
  async function add() {
    const totalMg = Number(cups) * Number(mg);
    const { error } = await supabase.from("coffee_logs").insert({ drink_date: todayKey(), coffee_name: name, cups, caffeine_mg: totalMg, note });
    if (error) return alert(error.message);
    setNote("");
  ...
  return <div className="space-y-4">
    <GlassCard className="bg-gradient-to-br from-amber-500/15 to-orange-500/10"><h2 className="text-2xl font-black">Coffee</h2><p className="mt-2 text-sm text-white/60">今日のカフェイン: <b>{todayMg}mg</b> / 目安{targetMg}mg。今の設定ならあと約 <b>{cupsLeft}杯</b>。</p><p className="mt-2 rounded-2xl bg-black/25 p-3 text-sm text-white/75">{status}</p></GlassCard>
    <GlassCard><h3 className="text-xl font-black">クイック記録</h3><div className="mt-3 grid gap-2 sm:grid-cols-5">{presets.map((p) => <button key={p.name} onClick={() => { setName(p.name); setMg(p.mg); }} className="rounded-2xl bg-white/10 px-3 py-3 text-sm font-black hover:bg-white/15">{p.name}<br /><span className="text-xs text-white/50">{p.mg}mg</span></button>)}</div><div className="mt-4 grid gap-3 sm:grid-cols-5"><Field value={name} onChange={(e) => setName(e.target.value)} /><Field type="number" value={cups} onChange={(e) => setCups(Number(e.target.value))} /><Field type="number" value={mg} onChange={(e) => setMg(Number(e.target.value))} /><Field type="number" value={targetMg} onChange={(e) => setTargetMg(Number(e.target.value))} placeholder="目安mg" /><Field placeholder="メモ" value={note} onChange={(e) => setNote(e.target.value)} /></div><div className="mt-3"><PrimaryButton onClick={add}>コーヒー記録</PrimaryButton></div></GlassCard>
    <GlassCard><h3 className="text-xl font-black">7日間カフェイン</h3><div className="mt-3 grid gap-2 sm:grid-cols-7">{last7.map((d) => <div key={d.key} className="rounded-2xl bg-black/25 p-3"><p className="text-[11px] text-white/45">{d.key.slice(5)}</p><p className="mt-1 text-lg font-black">{d.total}mg</p></div>)}</div></GlassCard>
...
function BudgetPanel({ snapshot, refreshSnapshot, themeKey }: PanelProps & { themeKey: ThemeKey }) {
  ...
  const [spendDate, setSpendDate] = useState(todayKey());
  ...
  const targetLeft = Math.max(0, Number(monthlyTarget || 0) - monthExpense);
  const targetRate = monthlyTarget ? Math.min(100, Math.round((monthExpense / Math.max(1, Number(monthlyTarget))) * 100)) : 0;
  ...
  async function addCharge() {
    ...
    const payload = { spend_date: spendDate, type: "charge", category: "チャージ", amount, wallet: chargeTo, source: chargeFrom, payment_method: chargeTo, memo: memo || `${chargeFrom} → ${chargeTo}` } as any;
  ...
  async function add() {
    ...
    const payload = { spend_date: spendDate, type, category, amount, wallet: accountLabel, source: type === "income" ? source : null, payment_method: accountLabel, memo: memo || null };
    const { error } = await supabase.from("budget_logs").insert(payload);
  ...
  async function addFromReceipt() {
    ...
    try {
      ...
      const { error } = await supabase.from("budget_logs").insert({ spend_date: spendDate, type: "expense", category: nextCategory, amount: nextAmount, wallet: nextWallet, payment_method: nextWallet, memo: nextMemo, image_url: imageDataUrl });
      if (error) {
        const fallback = await supabase.from("budget_logs").insert({ spend_date: spendDate, type: "expense", category: nextCategory, amount: nextAmount, wallet: nextWallet, payment_method: nextWallet, memo: nextMemo });
  ...
  async function useTemplate(t: BudgetFixedTemplate) {
    ...
    const { error } = await supabase.from("budget_logs").insert({ spend_date: spendDate, type: "expense", category: t.category, amount: Number(t.amount || 0), wallet: accountLabel, payment_method: accountLabel, memo: `固定費:${t.title}` });
  ...
  return <div className="space-y-4">
    <GlassCard className="bg-gradient-to-br from-emerald-500/15 to-cyan-500/10"><h2 className="text-2xl font-black">家計簿 Pro</h2><p className="mt-2 text-sm text-white/60">今月 収入 {yen(monthIncome)} / 支出 {yen(monthExpense)} / 固定費予定 {yen(fixedMonthly)}</p><p className="mt-4 text-4xl font-black">総資産 {yen(totalAssets)}</p><p className="mt-1 text-xs text-white/50">記録日を選べるようにして、月の予算目標・カテゴリ順位・残高ズレ確認をまとめて見える化したよ。</p></GlassCard>
    <div className="grid gap-3 lg:grid-cols-4"><GlassCard><p className="text-xs text-white/45">今月あと使える</p><p className="mt-2 text-3xl font-black text-cyan-100">{yen(remainingMonth)}</p><p className="mt-1 text-xs text-white/50">今日から月末まで1日 {yen(dailyBudget)} 目安</p></GlassCard><GlassCard><p className="text-xs text-white/45">月予算の残り</p><p className="mt-2 text-3xl font-black text-emerald-100">{yen(targetLeft)}</p><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-white/70" style={{ width: `${targetRate}%` }} /></div><p className="mt-1 text-xs text-white/50">目標 {yen(monthlyTarget)} / 消化 {targetRate}%</p></GlassCard><GlassCard><p className="text-xs text-white/45">固定費テンプレ合計</p><p className="mt-2 text-3xl font-black text-orange-100">{yen(fixedMonthly)}</p><p className="mt-1 text-xs text-white/50">家賃・サブスク・ジム代など</p></GlassCard><GlassCard><p className="text-xs text-white/45">カテゴリ警告</p><p className="mt-2 text-3xl font-black text-rose-100">{categoryWarnings.length}件</p><p className="mt-1 text-xs text-white/50">先月より増えた支出カテゴリ</p></GlassCard></div>
    <GlassCard><h3 className="text-xl font-black">家計簿の基本設定</h3><div className="mt-4 grid gap-3 sm:grid-cols-2"><DateField label="記録日" value={spendDate} onChange={(e) => setSpendDate(e.target.value)} /><Field type="number" placeholder="今月の支出目標" value={monthlyTarget} onChange={(e) => setMonthlyTarget(Number(e.target.value))} /></div></GlassCard>
    ...
    <GlassCard><h3 className="text-xl font-black">カテゴリランキング</h3><div className="mt-3 grid gap-2 sm:grid-cols-2">{Object.entries(currentCat).sort((a, b) => b[1] - a[1]).slice(0, 8).map(([name, value], index) => <div key={name} className="rounded-2xl bg-black/25 p-3"><p className="text-xs font-black text-white/45">#{index + 1}</p><p className="mt-1 font-black">{name}</p><p className="text-sm text-white/65">{yen(value)}</p></div>)}</div>{!Object.keys(currentCat).length && <p className="mt-2 text-sm text-white/55">今月の支出カテゴリはまだないよ。</p>}</GlassCard>
...
function BelongingsPanel({ snapshot, refreshSnapshot }: PanelProps) {
  ...
  return <div className="space-y-4">
    <GlassCard><h2 className="text-2xl font-black">持ち物</h2><p className="mt-2 text-sm text-white/60">ジム用・会社用・旅行用みたいにカードを作って、その中で持ち物をTodo式にチェックできるページ。写真も一緒に残せるよ。</p><button onClick={belongingsAi} disabled={belongingAiLoading} className="mt-4 w-full rounded-2xl bg-white/10 px-4 py-3 font-black disabled:opacity-50">持ち物をAI整理</button>{belongingAiText && <p className="mt-3 whitespace-pre-wrap rounded-2xl bg-black/25 p-4 text-sm leading-7 text-white/75">{belongingAiText}</p>}</GlassCard>
...
function RoutinesPanel({ snapshot, refreshSnapshot }: PanelProps) {
  ...
  async function check(r: Routine) {
    ...
    if (error) return alert("チェック失敗: " + error.message);
    if (data) {
      setUndoCheck(data as RoutineCheck);
...
function BrainDumpPanel({ refreshSnapshot, setPage }: { refreshSnapshot: (reason?: string) => Promise<void>; setPage: (p: PageKey) => void }) {
  ...
  const [saving, setSaving] = useState(false);
  const [sortBanner, setSortBanner] = useState("");
  const sortBannerTimer = useRef<number | null>(null);
...
function GlobalSearchModal({ snapshot, setPage, onClose }: { snapshot: Snapshot | null; setPage: (p: PageKey) => void; onClose: () => void }) {
  ...
  const results = useMemo(() => collectGlobalSearchResults(snapshot, debouncedQuery), [snapshot, debouncedQuery]);
